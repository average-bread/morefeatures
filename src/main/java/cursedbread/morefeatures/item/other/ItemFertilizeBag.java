...
import static net.minecraft.core.block.BlockLogicFarmland.isFertilized;
import static net.minecraft.core.block.BlockLogicFarmland.setFertilized;
...
public class ItemFertilizeBag extends ItemQuiver {
	...
	public boolean onUseItemOnBlock(ItemStack itemstack, @Nullable Player player, World world, int blockX, int blockY, int blockZ, Side side, double xPlaced, double yPlaced) {
		...
		int j1;
		//normal blocks
		if (player.getHeldItem().getMetadata() < 192){
			...
			if (Blocks.blocksList[id] != null && Blocks.blocksList[id].hasTag(BlockTags.GROWS_FLOWERS)) {
			...
			//crops
			if (id == Blocks.CROPS_WHEAT.id()){
				if (world.getBlockMetadata(blockX, blockY, blockZ) >= 7) {
					return false;
				} else {
					if (!world.isClientSide) {
						world.setBlockMetadataWithNotify(blockX, blockY, blockZ, 7);
						if (player == null || player.getGamemode().consumeBlocks()) {
							player.getHeldItem().damageItem(1, player);
						}
					}
			...
			if (id == Blocks.CROPS_PUMPKIN.id()){
				if (world.getBlockMetadata(blockX, blockY, blockZ) >= 5) {
					return false;
				} else {
					if (!world.isClientSide) {
						world.setBlockWithNotify(blockX, blockY, blockZ, Blocks.PUMPKIN.id());
						if (player == null || player.getGamemode().consumeBlocks()) {
							player.getHeldItem().damageItem(1, player);
						}
					}
			...
			if (id == FeaturesBlocks.fluxCropws.id()){
				if (world.getBlockMetadata(blockX, blockY, blockZ) >= 7) {
					return false;
				} else {
					if (!world.isClientSide) {
			...
			if (id == Blocks.LEAVES_CHERRY_FLOWERING.id()){
				int meta = world.getBlockMetadata(blockX, blockY, blockZ);
			...
			//plant blocks
			if (id == Blocks.CACTUS.id()){
				if (!world.isClientSide) {
					int l;
					for(l = 1; world.getBlockId(blockX, blockY + l, blockZ) == Blocks.CACTUS.id(); ++l) {
					}

					if (world.isAirBlock(blockX, blockY + l, blockZ)) {
						world.setBlockWithNotify(blockX, blockY + l, blockZ, Blocks.CACTUS.id());
					}
					if (player == null || player.getGamemode().consumeBlocks()) {
						player.getHeldItem().damageItem(1, player);
					}
			...
			if (id == Blocks.SUGARCANE.id()){
				if (!world.isClientSide) {
					int l;
					for(l = 1; world.getBlockId(blockX, blockY + l, blockZ) == Blocks.SUGARCANE.id(); ++l) {
					}

					if (world.isAirBlock(blockX, blockY + l, blockZ)) {
						world.setBlockWithNotify(blockX, blockY + l, blockZ, Blocks.SUGARCANE.id());
					}
					if (player == null || player.getGamemode().consumeBlocks()) {
						player.getHeldItem().damageItem(1, player);
					}
			...
			if (world.getBlock(blockX, blockY, blockZ).getLogic().getClass() == BlockLogicFlowerStackable.class){
				...
				if (!world.isClientSide) {
					...
					for(j1 = 0; j1 < 128; ++j1) {
						...
						if (world.getBlockId(k1, l1, i2) == 0 && (double)rand.nextFloat() > 0.75) {
							world.setBlockWithNotify(k1, l1, i2, world.getBlockId(blockX, blockY, blockZ));
						}
					}
			...
			if (world.getBlock(blockX, blockY, blockZ).getLogic().getClass() == BlockLogicMoss.class){
				if (!world.isClientSide) {
					if (player == null || player.getGamemode().consumeBlocks()) {
						player.getHeldItem().damageItem(1, player);
					}
			...
			//saplings
			if (world.getBlockId(blockX, blockY, blockZ) == Blocks.SAPLING_OAK.id()){
				if (!world.isClientSide) {
					...
					if (!((WorldFeature)treeSmall).place(world, world.rand, blockX, blockY, blockZ) && !((WorldFeature)treeBig).place(world, world.rand, blockX, blockY, blockZ)) {
						world.setBlock(blockX, blockY, blockZ, Blocks.SAPLING_OAK.id());
					}
					if (player == null || player.getGamemode().consumeBlocks()) {
						player.getHeldItem().damageItem(1, player);
					}
			...
			if (world.getBlockId(blockX, blockY, blockZ) == Blocks.SAPLING_BIRCH.id()){
				if (!world.isClientSide) {
					...
					world.setBlock(blockX, blockY, blockZ, 0);
					if (!((WorldFeature)treeSmall).place(world, world.rand, blockX, blockY, blockZ) && !((WorldFeature)treeBig).place(world, world.rand, blockX, blockY, blockZ)) {
						world.setBlock(blockX, blockY, blockZ, Blocks.SAPLING_BIRCH.id());
					}
					if (player == null || player.getGamemode().consumeBlocks()) {
						player.getHeldItem().damageItem(1, player);
					}
			...
			if (world.getBlockId(blockX, blockY, blockZ) == Blocks.SAPLING_CACAO.id()){
				if (!world.isClientSide) {
					...
					world.setBlock(blockX, blockY, blockZ, 0);
					if (!((WorldFeature)treeSmall).place(world, world.rand, blockX, blockY, blockZ) && !((WorldFeature)treeBig).place(world, world.rand, blockX, blockY, blockZ)) {
						world.setBlock(blockX, blockY, blockZ, Blocks.SAPLING_CACAO.id());
					}
					if (player == null || player.getGamemode().consumeBlocks()) {
						player.getHeldItem().damageItem(1, player);
					}
			...
			if (world.getBlockId(blockX, blockY, blockZ) == Blocks.SAPLING_CHERRY.id()){
				if (!world.isClientSide) {
					...
					world.setBlock(blockX, blockY, blockZ, 0);
					if (!((WorldFeature)treeSmall).place(world, world.rand, blockX, blockY, blockZ) && !((WorldFeature)treeBig).place(world, world.rand, blockX, blockY, blockZ)) {
						world.setBlock(blockX, blockY, blockZ, Blocks.SAPLING_CHERRY.id());
					}
					if (player == null || player.getGamemode().consumeBlocks()) {
						player.getHeldItem().damageItem(1, player);
					}
			...
			if (world.getBlockId(blockX, blockY, blockZ) == Blocks.SAPLING_EUCALYPTUS.id()){
				world.setBlock(blockX, blockY, blockZ, 0);
				...
				if (!((WorldFeature)tree).place(world, world.rand, blockX, blockY, blockZ)) {
					world.setBlock(blockX, blockY, blockZ, Blocks.SAPLING_EUCALYPTUS.id());
				}

				return true;
			}
			if (world.getBlockId(blockX, blockY, blockZ) == Blocks.SAPLING_PALM.id()){
				boolean big = world.getBlockId(blockX, blockY - 1, blockZ) != Blocks.SAND.id();
				(new WorldFeatureTreePalm(Blocks.LOG_PALM, Blocks.LEAVES_PALM, big, true, false)).place(world, world.rand, blockX, blockY, blockZ);

				return true;
			}
			if (world.getBlockId(blockX, blockY, blockZ) == Blocks.SAPLING_PINE.id()){
				...
				world.setBlock(blockX, blockY, blockZ, 0);
				if (!((WorldFeature)treeSmall).place(world, world.rand, blockX, blockY, blockZ) && !((WorldFeature)treeBig).place(world, world.rand, blockX, blockY, blockZ)) {
					world.setBlock(blockX, blockY, blockZ, Blocks.SAPLING_PINE.id());
				}

				return true;
			}
			if (world.getBlockId(blockX, blockY, blockZ) == Blocks.SAPLING_OAK_RETRO.id()){
				...
				world.setBlock(blockX, blockY, blockZ, 0);
				if (!((WorldFeature)treeSmall).place(world, world.rand, blockX, blockY, blockZ) && !((WorldFeature)treeBig).place(world, world.rand, blockX, blockY, blockZ)) {
					world.setBlock(blockX, blockY, blockZ, Blocks.SAPLING_OAK_RETRO.id());
				}

				return true;
			}
			if (world.getBlockId(blockX, blockY, blockZ) == Blocks.SAPLING_SHRUB.id()){
				world.setBlock(blockX, blockY, blockZ, 0);
				WorldFeature tree = new WorldFeatureTreeShrub(Blocks.LEAVES_SHRUB.id(), Blocks.LOG_OAK.id());
				if (!((WorldFeature)tree).place(world, world.rand, blockX, blockY, blockZ)) {
					world.setBlock(blockX, blockY, blockZ, Blocks.SAPLING_SHRUB.id());
				}

				return true;
			}
			if (world.getBlockId(blockX, blockY, blockZ) == Blocks.SAPLING_THORN.id()){
				if (!world.isClientSide) {
					world.setBlock(blockX, blockY, blockZ, 0);
					WorldFeature treeFeature = new WorldFeatureTreeThorn(1, Blocks.LOG_THORN.id(), 0, Blocks.LEAVES_THORN.id(), 0);
					if (!((WorldFeature)treeFeature).place(world, world.rand, blockX, blockY, blockZ)) {
						world.setBlock(blockX, blockY, blockZ, Blocks.SAPLING_THORN.id());
					}
				}
				return true;
			}
