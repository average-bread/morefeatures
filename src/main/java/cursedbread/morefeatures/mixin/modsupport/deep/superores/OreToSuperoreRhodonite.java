...
@Mixin(value = BlockLogicRhodoniteOre.class, remap = false)

public class OreToSuperoreRhodonite {
		...
		if (FeaturesMain.deepOn){
			if (FeaturesBlocks.superoresEnabled == 1) {
				if (g <= FeaturesMain.superoreChance - 1){
					if (ore == DEEPBlocks.netherrackRhodoniteOre.id()){
						world.setBlock(x, y, z, FeaturesBlocks.super_Rhodonite_Ore.id());
					}
				}
			}
		}
